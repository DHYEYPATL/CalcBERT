...
const PrepayResultScreen = () => {
  ...
  useEffect(() => {
    const fetchPrepayCheck = async () => {
      ...
      try {
        ...
        console.log('Prepay check response:', JSON.stringify(response, null, 2))
        setResult(response)
      } catch (err: any) {
  ...
  const handleCorrectCategory = async (correctCategory: string) => {
    ...
    try {
      ...
      Alert.alert('Success', `Category corrected to "${correctCategory}". Thank you for the feedback!`, [
        {
          text: 'OK',
          onPress: () => {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.card}>
        ...
        {(confidence < 50 || category === 'Unknown' || category === 'unknown') && (
          <TouchableOpacity
